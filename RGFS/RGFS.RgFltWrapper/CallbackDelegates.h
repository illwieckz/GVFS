...
namespace RgLib
{
    ...
    /// for creating the placeholder for the filename requested.  Returning from callback signals the RgFlt driver that all 
    /// information needed to create a placeholder was provided to RgFlt by a successful call to WritePlaceholderInformation.
    ///
    /// If this callback is being handled asynchrnously, CompleteCommand should be called (passing in commandId) 
    /// after calling WritePlaceholderInformation to notify RgFlt that processing of this command is complete.
    /// </remarks>
    ...
    /// Returning from the callback signals the RgFlt driver that all file stream content has been provided to RgFlt by a successful call to WriteFile.
    /// </remarks>
    ...
    /// <returns>Success if callback succeeded, appropriate error otherwise</returns>
    /// <remarks>
    /// Returning from the callback signals the RgFlt driver that the provider has completed all necessary 
    /// bookkeeping and the write operation can proceed.
    /// 
    /// Note -
    /// Returning an error from the callback will cause RgFlt driver to fail to proceed and therefore fail the user request
    ...
    ///     a) A callback with the path to the parent directory will be sent
    /// 
    /// RgFlt guarantees that below properties for this callback is true at all the times -
    ...
    /// 
    /// RgFlt won't send this notification if someone tries to undo the delete by setting 
    ...
    ///
    /// The provider won't receive the rename notification if RgFlt decided to fail the request.
