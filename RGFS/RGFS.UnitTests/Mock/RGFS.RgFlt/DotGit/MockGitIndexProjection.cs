...
namespace RGFS.UnitTests.Mock.RGFS.RgFlt.DotGit
{
    public class MockGitIndexProjection : GitIndexProjection
    {
        ...
        public override bool TryGetProjectedItemsFromMemory(string folderPath, out IEnumerable<RGFltFileInfo> projectedItems)
        {
            if (this.EnumerationInMemory)
            {
                projectedItems = this.projectedFiles.Select(name => new RGFltFileInfo(name, size: 0, isFolder: false)).ToList();
                return true;
            }
        ...
        public override IEnumerable<RGFltFileInfo> GetProjectedItems(string folderPath, CancellationToken cancellationToken)
        {
            ...
            return this.projectedFiles.Select(name => new RGFltFileInfo(name, size: 0, isFolder: false)).ToList();
        }
        ...
        public override RGFltFileInfo GetProjectedRGFltFileInfoAndSha(CancellationToken cancellationToken, string virtualPath, out string parentFolderPath, out string sha)
        {
            ...
            if (this.projectedFiles.Contains(virtualPath))
            {
                ...
                return new RGFltFileInfo(childName, size: 0, isFolder: false);
            }
