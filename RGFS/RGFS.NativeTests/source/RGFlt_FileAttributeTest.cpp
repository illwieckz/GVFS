#include "stdafx.h"
#include "RGFlt_FileAttributeTest.h"
...
static const std::string TEST_ROOT_FOLDER("\\RGFlt_FileAttributeTest");

bool RGFlt_ModifyFileInScratchAndCheckLastWriteTime(const char* virtualRootPath)
{
    try
    {
        ...
        // NOTE: This is slightly different than the validate in RGFlt, which tests if the scratch time is different than the layer time
...
bool RGFlt_FileSize(const char* virtualRootPath)
{
    try
    {
...
bool RGFlt_ModifyFileInScratchAndCheckFileSize(const char* virtualRootPath)
{
    try
    {
...
bool RGFlt_FileAttributes(const char* virtualRootPath)
{
    try
    {
