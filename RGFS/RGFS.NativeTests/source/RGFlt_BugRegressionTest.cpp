#include "stdafx.h"
#include "RGFlt_BugRegressionTest.h"
...
static const std::string TEST_ROOT_FOLDER("\\RGFlt_BugRegressionTest");

bool RGFlt_ModifyFileInScratchAndDir(const char* virtualRootPath)
{
    // For bug #7700746 - File size is not updated when writing to a file projected from RGFlt app (e.g. RGFS, test app)

    try
    {
        std::string testScratch = virtualRootPath + TEST_ROOT_FOLDER + std::string("\\RGFlt_ModifyFileInScratchAndDir\\");
...
bool RGFlt_RMDIRTest1(const char* virtualRootPath)
{
    // Bug #7703883 - RgFlt: RMDIR /s against a partial folder returns directory not empty error
...
bool RGFlt_RMDIRTest2(const char* virtualRootPath)
{
    try
    {
        // layer: 1
...
bool RGFlt_RMDIRTest3(const char* virtualRootPath)
{
    try
    {
        // layer: 1, 3
...
bool RGFlt_RMDIRTest4(const char* virtualRootPath)
{
    try
    {
        // layer: 2
...
bool RGFlt_RMDIRTest5(const char* virtualRootPath)
{
    try
    {
        // layer: 1, 2, 4
...
bool RGFlt_DeepNonExistFileUnderPartial(const char* virtualRootPath)
{
    try
    {
...
bool RGFlt_SupersededReparsePoint(const char* virtualRootPath)
{
    try
    {
