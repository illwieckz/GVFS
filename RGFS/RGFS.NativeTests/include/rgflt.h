...
// sdktools\CoreBuild\RGFlt\rgflt.h
...
#define RGFLT_PLACEHOLDER_ID_LENGTH      128
//
// Structure that uniquely identifies the version of the attributes, file streams etc for a placeholder file
//

typedef struct _RGFLT_PLACEHOLDER_VERSION_INFO {

    UCHAR                               EpochID[RGFLT_PLACEHOLDER_ID_LENGTH];

    UCHAR                               ContentID[RGFLT_PLACEHOLDER_ID_LENGTH];

} RGFLT_PLACEHOLDER_VERSION_INFO, *PRGFLT_PLACEHOLDER_VERSION_INFO;
...
typedef struct _GV_REPARSE_INFO {
    ...
    RGFLT_PLACEHOLDER_VERSION_INFO versionInfo;
