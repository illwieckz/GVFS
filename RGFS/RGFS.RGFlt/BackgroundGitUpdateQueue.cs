...
namespace RGFS.RGFlt
{
    public class BackgroundGitUpdateQueue : FileBasedCollection
    {
        ...
        private readonly ConcurrentQueue<KeyValuePair<long, RGFltCallbacks.BackgroundGitUpdate>> data = new ConcurrentQueue<KeyValuePair<long, RGFltCallbacks.BackgroundGitUpdate>>();
        ...
        public static bool TryCreate(ITracer tracer, string dataDirectory, PhysicalFileSystem fileSystem, out BackgroundGitUpdateQueue output, out string error)
        {
            ...
            if (!output.TryLoadFromDisk<long, RGFltCallbacks.BackgroundGitUpdate>(
        ...
        public void EnqueueAndFlush(RGFltCallbacks.BackgroundGitUpdate value)
        {
            try
            {
                KeyValuePair<long, RGFltCallbacks.BackgroundGitUpdate> kvp = new KeyValuePair<long, RGFltCallbacks.BackgroundGitUpdate>(
        ...
        public void DequeueAndFlush(RGFltCallbacks.BackgroundGitUpdate expectedValue)
        {
            try
            {
                KeyValuePair<long, RGFltCallbacks.BackgroundGitUpdate> kvp;
        ...
        public bool TryPeek(out RGFltCallbacks.BackgroundGitUpdate value)
        {
            try
            {
                KeyValuePair<long, RGFltCallbacks.BackgroundGitUpdate> kvp;
                ...
                value = default(RGFltCallbacks.BackgroundGitUpdate);
                return false;
            }
            catch (Exception e)
            {
        ...
        private bool TryParseAddLine(string line, out long key, out RGFltCallbacks.BackgroundGitUpdate value, out string error)
        {
        ...
        private string Serialize(RGFltCallbacks.BackgroundGitUpdate input)
        {
            return ((int)input.Operation) + ValueTerminator + input.VirtualPath + ValueTerminator + input.OldVirtualPath;
        }

        private bool TryDeserialize(string line, out RGFltCallbacks.BackgroundGitUpdate value)
        {
            ...
            RGFltCallbacks.BackgroundGitUpdate.OperationType operationType;
            ...
            value = new RGFltCallbacks.BackgroundGitUpdate(
                operationType,
                parts[1],
                parts[2]);
        ...
        private void AddParsedEntry(long key, RGFltCallbacks.BackgroundGitUpdate value)
        {
            this.data.Enqueue(new KeyValuePair<long, RGFltCallbacks.BackgroundGitUpdate>(key, value));
