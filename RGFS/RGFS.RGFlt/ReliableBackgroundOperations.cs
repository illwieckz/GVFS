...
namespace RGFS.RGFlt
{
    public class ReliableBackgroundOperations : IDisposable
    {
        ...
        private Func<RGFltCallbacks.BackgroundGitUpdate, CallbackResult> callback;
        private Func<CallbackResult> postCallback;

        public ReliableBackgroundOperations(
            RGFSContext context,
            Func<CallbackResult> preCallback,             
            Func<RGFltCallbacks.BackgroundGitUpdate, CallbackResult> callback,
        ...
        public virtual void Enqueue(RGFltCallbacks.BackgroundGitUpdate backgroundOperation)
        {
        ...
        {
            RGFltCallbacks.BackgroundGitUpdate backgroundOperation;
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    while (this.backgroundOperations.TryPeek(out backgroundOperation))
                    {
                        ...
                        if (this.isStopping)
                        {
                            // If we are stopping, then RGFlt has already been shut down
                            // Some of the queued background tasks may require RGFlt, and so it is unsafe to
